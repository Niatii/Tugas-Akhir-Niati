      ...
      <div class="login-card">
        ...
        <div class="full-width bg-blue-2 q-pa-xl" style="border-radius: 12px">
          <q-form @submit.prevent="nextStep" autocomplete="on">
            ...
            <span v-if="errors.nim" class="error-text q-mb-md"> {{ errors.nim }} </span>
            ...
            >
              ...
              <template v-slot:no-option>
                <q-item>
                  <q-item-section class="text-grey"> Tidak ada data saat ini </q-item-section>
                </q-item>
              </template>
            </q-select>
            <span v-if="errors.jurusan" class="error-text q-mb-md"> {{ errors.jurusan }} </span>
...
watch(jurusan, async (val, oldVal) => {
  ...
  try {
    programStudiOptions.value =
      await masterApi.getProdiByJurusan(val.id)
